...
# =====================================================
# PHASE 3: POSITION OUTPUT SECTION
# =====================================================
st.divider()
st.header("📍 Position Output")
st.markdown("Calculate and export exact coordinates of atoms and intersection sites.")

# Session state for position data persistence
if "structure" not in st.session_state:
    st.session_state.structure = None

col_pos = st.columns(2)
with col_pos[0]:
    calc_btn = st.button("📊 Calculate Positions for Current Settings", type="primary", key="calc_pos_btn")

with col_pos[1]:
    if st.session_state.structure is not None:
        st.success("✓ Positions calculated")
...
if st.session_state.structure is not None:
    ...
    # Preview tables
    tab1, tab2, tab3 = st.tabs(["Metal Atoms", "Intersections", "3D + Export"])
    ...
    with tab3:
        st.subheader("3D View with Positions")
        
        # Create enhanced 3D plot with intersections
        a_vec, b_vec, c_vec = lattice_vectors(p)
        fig_pos = go.Figure()
        ...
        for idx, sub_name in enumerate(unique_sublattices):
            ...
            fig_pos.add_trace(go.Scatter3d(
                ...
                legendgroup=f"metal{idx}",
            ))
        
        # Add intersections colored by multiplicity
        if len(structure.intersections.fractional) > 0:
            ...
            fig_pos.add_trace(go.Scatter3d(
                ...
                hovertemplate='<b>Intersection</b><br>Position: (%{x:.3f}, %{y:.3f}, %{z:.3f})<br>%{text}<extra></extra>'
            ))
        
        fig_pos.update_scenes(aspectmode="data")
        ...
        st.info("💡 Data persists in session. Adjust s or N values and recalculate to update positions.")
