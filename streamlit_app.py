...
k_samples = st.sidebar.select_slider(
    "Samples per pair-circle", options=[4, 8, 12, 16, 24, 32], value=8
)
...
def edit_sublattice(idx: int):
    cols = st.columns(3)
    with cols[0]:
        bravais = st.selectbox(
            f"Bravais {idx}", bravais_choices, index=2 if idx == 1 else 0, key=f"brv{idx}"
        )
        alpha_ratio = st.number_input(
            f"α ratio {idx}", 0.01, 3.0, 1.0, 0.01, key=f"ar{idx}"
        )
...
param = st.selectbox(
    "Parameter to scan", ["a", "b_ratio", "c_ratio", "alpha", "beta", "gamma"], index=2
)
...
if param in ("b_ratio", "c_ratio"):
    ...
    vmin, vmax = 60.0, 140.0
else:
    vmin, vmax = 0.5, 1.5

vmin = st.number_input("Param min", value=vmin)
vmax = st.number_input("Param max", value=vmax)
...
if st.button("Run scan"):
    ...
    def clone_params(base: LatticeParams, **kw) -> LatticeParams:
        d = dict(
            a=base.a,
            b_ratio=base.b_ratio,
            c_ratio=base.c_ratio,
            alpha=base.alpha,
            beta=base.beta,
            gamma=base.gamma,
        )
...
The engine uses pair-circle sampling, a fast cell-list neighbour search, clustering,
and bisection to find s*_N.  
